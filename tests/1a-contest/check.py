...
def get_key(rec, keyword):
    words = rec.split(' ')
    if keyword >= len(words) - 1:
        return words[-1].replace('\n', '')
    else:
        return words[keyword]

# return a printable list of records, possibly truncated
def trunc_list(l, keyword):
   n = 3
   if len(l) <= n:
      return ", ".join([str(get_key(i, keyword)) for i in l])
   return ", ".join([str(get_key(i, keyword)) for i in l[:n]] + ["..."])

# check that sorted_file is sorted and contains the same records as orig_file
def check_sorted(sorted_file, orig_file, keyword=0):
   count = Counter()
   ...
   prevkey = None
   sorted_lines = StringIO.StringIO(sorted_file)
   while True:
       rec = sorted_lines.readline()
       if len(rec) == 0:
           break
       key = get_key(rec, keyword)
   ...
   if len(count) > 0:
      ...
      if len(missing) > 0:
         r.append("missing " + str(len(missing)) + " records "
               + "(" + trunc_list(missing, keyword) + ")")
      if len(extra) > 0:
         r.append("extra " + str(len(extra)) + " records "
               + "(" + trunc_list(extra, keyword) + ")")
      return (False, "\n".join(r))
